...
ui <- fluidPage(
   ...
   fluidRow(
     column(3, 
            ...
            checkboxInput("fewer_colors", "Fewer colors", FALSE, width = "100%")
     ),
     column(9,
...
server <- function(input, output, session){
  ...
  tile_group <- reactive(ifelse(input$fewer_colors, 2, 1))
  tile_file <- reactive(gsub("\\.tif", "", basename(mapfile())))
  tiles <- reactive({ 
    file.path("https://leonawicz.github.io/tempdocs/deltamaps", tile_group(), tile_file(), "{z}/{x}/{y}.png")
  })
  ...
  observe({
    leafletProxy("Map") %>% removeTiles("raslayer") %>% removeControl("legend") %>%
      addTiles(tiles(), options = providerTileOptions(opacity = input$ras_op), layerId = "raslayer") %>%
